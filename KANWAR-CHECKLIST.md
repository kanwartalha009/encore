...
- [ ] **4. Email sending** — resend.com → create API key → add your sending domain → add the DNS records it gives you → wait for "Verified". In Railway → encore → Variables add `ENCORE_EMAIL_API_KEY` and `ENCORE_EMAIL_FROM="Store <notify@<your-verified-domain>>"` (the app reads ENCORE_EMAIL_API_KEY / ENCORE_EMAIL_FROM — not RESEND_API_KEY; earlier docs had the wrong names). Railway redeploys automatically.
